...
class Backend::LocalsControllerTest < ActionController::TestCase
  ...
  test "should show local" do
    get :show, :id => locals(:one).id
    assert_response :success
  end
 
  test "should get edit" do
    get :edit, :id => locals(:one).id
    assert_response :success
  end
 
  test "should update local" do
    put :update, :id => locals(:one).id, :local => { }
    assert_redirected_to local_path(assigns(:local))
  end
 
  test "should destroy local" do
    assert_difference('Local.count', -1) do
      delete :destroy, :id => locals(:one).id
    end
